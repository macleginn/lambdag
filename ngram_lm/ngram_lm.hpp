...
// Assuming trie.hpp provides the Trie class declaration and implementation
#include "trie.hpp"
...
public:
    ...
    double getNgramProbability(
        ...
        // If not, direct access like .back() or [size-2] can be unsafe if vectors are shorter.
        // The Python code would raise IndexError. C++ would be undefined behavior.
        // For safety, we should check sizes if the contract with Trie::getCounts isn't strict.
        // Given ngram.size() >= 2 at this point.
    ...
    std::vector<double> getNgramProbabilities(
        ...
        // Use a mutex to protect access to the shared task queue
        ...
        auto worker_lambda = [&]() -> std::vector<std::pair<size_t, double>> {
            ...
            while (true) {
                ...
                // Process the task: calculate probability
                // Capture method and discount by value for the lambda inside getNgramProbability if it were also async
                // Here, getNgramProbability is called directly.
        ...
        for (auto& fut : futures) {
            try {
                ...
                // Handle or rethrow. For simplicity, rethrowing.
                // Consider logging the error or collecting partial results.
